...
void AC_Avoid::adjust_velocity_z(float kP, float accel_cmss, float& climb_rate_cms)
{
	...
	{
//		printf("AC_Avoid::adjust_velocity_z ok %4.2f\n", proximity_alt_diff_m);
	...
	//printf("AC_Avoid::adjust_velocity_z error\n");
...
void AC_Avoid::adjust_velocity_polygon(float kP, float accel_cmss, Vector2f &desired_vel, const Vector2f* boundary, uint16_t num_points, bool earth_frame, float margin)
{
    ...
    if (!earth_frame) {
        safe_vel.x = desired_vel.y * _ahrs.sin_yaw() + desired_vel.x * _ahrs.cos_yaw(); // forward
        safe_vel.y = desired_vel.y * _ahrs.cos_yaw() - desired_vel.x * _ahrs.sin_yaw(); // right
    }

    // calc margin in cm
    float margin_cm = MAX(margin * 100.0f, 0);

	//	modified by ZhangYong 20171004
	//	maybe this is a bug
    ...
    for (i = 0, j = num_points-1; i < num_points; j = i++) {
        ...
        /*printf("[%d].x = %4.2f [%d].y = %4.2f, [%d].x = %4.2f [%d].y = %4.2f, d.x = %4.2f, d.y = %4.2f\n", \
									j, boundary[j].x, j, boundary[j].y, \
									i, boundary[i].x, i, boundary[i].y, \
									limit_direction.x, limit_direction.y);
		*/
...
void AC_Avoid::get_proximity_roll_pitch_pct(float &roll_positive, float &roll_negative, float &pitch_positive, float &pitch_negative)
{
    ...
    for (uint8_t i=0; i<obj_count; i++) {
		...
		{
/*			printf("get_proximity_roll_pitch_pct [%d] deg: %4.1f, dist: %4.1f\n", \
																				i, \
																				ang_deg, \
																				dist_m);
*/         	if (dist_m < _dist_max) 
         	{
				...
				//	added by ZhangYong 20170930 to do.....

			/*	printf("pct %4.2f, r %4.2f, p %4.2f, rp %4.2f, rn %4.2f, pp %4.2f, pn %4.2f\n",
																lean_pct, \
																roll_pct, \
																pitch_pct, \
																roll_positive, \
